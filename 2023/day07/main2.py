...
def hand_type(hand: str) -> int:
    ...
    if ('J' in counts) and (len(counts) > 1):  # make sure hand does not consist of five J's
        # The best choice is always to make J match most common card
        num = counts['J']
        del counts['J']
        try:
            counts[counts.most_common(1)[0][0]] += num
